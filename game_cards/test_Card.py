...
class TestCard(TestCase):
    ...
    def test__init__valid_edge_value(self):
        """Function that tests if the __init__ of Card worked when the values are in the edge of validations"""
        card=Card(1,2)
    ...
    def test__gt__valid(self):
        """Function that tests the valid boolean responses to different cases of other card"""
        other=Card(2,2)
    ...
    def test__gt__valid_other_same_as_card(self):
        """Function that tests that indeed returns False if they are the same card"""
        other= Card(5,3)
        self.assertFalse(self.card.__gt__(other))
    ...
    def test__gt__invalid_other_type(self):
        """Function that tests if the program raise with Type error when gets classes of other that are not of the Card class"""
        with self.assertRaises(TypeError):
            other = 2
    ...
    def test__eq__valid_eq_suit(self):
        """Function that tests return False when the value of card and other are different and the suit is the same"""
        other = Card(6, 3)
        self.assertFalse(self.card.__eq__(other))
        other = Card(10, 3)
        self.assertFalse(self.card.__eq__(other))
    ...
    def test_suit_name_valid(self):
        """Function that tests that suit_name take the number of the suit of the card and change it to its name"""
    ...
    def test_suit_name_invalid_card_suit(self):
        """Function that tests the program raise with Value error when gets card with invalid suit"""
        with self.assertRaises(ValueError):
            card = Card(5, 5)
            card.suit_name()
        with self.assertRaises(ValueError):
    ...
    def test_value_name_valid(self):
        """Function that tests that value_name take the number of the value of the card and change it to its name"""
    ...
    def test_value_name_invalid_card_value(self):
        """Function that tests the program raise with Value error when gets card with invalid value"""
        with self.assertRaises(ValueError):
            card = Card(14, 3)
            card.suit_name()
        with self.assertRaises(ValueError):
